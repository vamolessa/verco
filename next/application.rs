...
use crate::platform::{Key, PlatformRequest, ProcessHandle, ProcessTag};
...
impl ServerApplication {
    fn run_application(
        ...
        'event_loop: loop {
            ...
            loop {
                match event {
                    ApplicationEvent::Idle => {
                        editor.on_idle(&mut clients, platform)
                    }
                    ...
                    ApplicationEvent::ConnectionOutput { handle, buf } => {
                        let mut events = client_event_receiver
                            .receive_events(handle, buf.as_bytes());
                        while let Some(event) =
                            events.next(&client_event_receiver)
                        {
                    ...
                    ApplicationEvent::ProcessOutput { tag, buf } => editor
                        .on_process_output(
                            platform,
                            &mut clients,
                            tag,
            ...
            for c in clients.iter() {
                ...
                let mut buf = platform.buf_pool.acquire();
                let write =
                    buf.write_with_len(ServerEvent::display_header_len());
...
impl<'stdout> ClientApplication<'stdout> {
    ...
    pub fn init(&mut self, args: Args) -> &[u8] {
        ...
        if !self.is_pipped && !args.as_focused_client {
            ClientEvent::Key(self.target_client, Key::None)
    ...
    pub fn update<'a>(
        ...
        if let Some((width, height)) = resize {
            ClientEvent::Resize(width as _, height as _)
                .serialize(&mut self.server_write_buf);
        }

        for key in keys {
            ClientEvent::Key(self.target_client, *key)
                .serialize(&mut self.server_write_buf);
        }
            ...
            loop {
                ...
                match ServerEvent::deserialize(&mut read_slice) {
                    ...
                    Err(DeserializeError::InsufficientData) => {
                        let read_len =
                            self.server_read_buf.len() - previous_slice.len();
...
pub fn set_panic_hook() {
    static mut ORIGINAL_PANIC_HOOK: Option<
        Box<dyn Fn(&panic::PanicInfo) + Sync + Send + 'static>,
    > = None;
